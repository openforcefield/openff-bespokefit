...
class BespokeSmirksSchema(SchemaBase):
    """
    ...
    def __eq__(self, other: "BespokeSmirksSchema"):
        """
        Compare two smirks patterns and determine if they are equivalent.

        First we compare the string if they do not match we have to do a graph isomorphic
        check specific to the parameter type.
        """
...
class ValidatedBespokeSmirks(BespokeSmirksSchema, abc.ABC):
...
class BespokeAtomSmirks(ValidatedBespokeSmirks):
    """
    ...
    def _validate_smirks(cls, smirks: str) -> str:
        return validate_smirks(smirks=smirks, expected_tags=1)
...
class BespokeBondSmirks(ValidatedBespokeSmirks):
    ...
    @validator("smirks")
    def _validate_smirks(cls, smirks: str) -> str:
        return validate_smirks(smirks=smirks, expected_tags=2)
...
class BespokeAngleSmirks(ValidatedBespokeSmirks):
    ...
    @validator("smirks")
    def _validate_smirks(cls, smirks: str) -> str:
        return validate_smirks(smirks=smirks, expected_tags=3)
...
class BespokeTorsionTerm(SchemaBase):
    periodicity: str
    phase: str
    k: str
    idivf: str = "1.0"
    ...
    def _validate_periodicity(cls, p: str) -> str:
        if 1 <= int(p) <= 6:
            ...
            raise ValueError("Periodicity must be between 1 and 6.")
...
class BespokeTorsionSmirks(BespokeSmirksSchema):
    ...
    @validator("smirks")
    def _validate_smirks(cls, smirks: str) -> str:
        return validate_smirks(smirks=smirks, expected_tags=4)
    ...
    def __eq__(self, other):
        return super(BespokeTorsionSmirks, self).__eq__(other=other)
    ...
    def add_torsion_term(self, term: Union[str, BespokeTorsionTerm]) -> None:
        if isinstance(term, str):
            # make a torsion term and add it
            periodicity = term.split("k")[-1]
            new_term = BespokeTorsionTerm(periodicity=periodicity)
        else:
            new_term = term
...
BespokeSmirksParameter = Union[
    BespokeAtomSmirks, BespokeBondSmirks, BespokeAngleSmirks, BespokeTorsionSmirks
]
