...
def test_initial_parameter_values(bespoke_optimization_results):
    ...
    parameter_values = bespoke_optimization_results.initial_parameter_values.values()
...
def test_refit_parameter_values(bespoke_optimization_results):
    ...
    refit_parameter_values = (
        bespoke_optimization_results.refit_parameter_values.values()
    )
    ...
    assert all(
        isinstance(parameter, unit.Quantity)
        for x in refit_parameter_values
        for parameter in x.values()
    )
