...
def test_atom_smirks(smirks_data):
    ...
    if error is None:
        smirks = AtomSmirks(smirks=smirks_pattern, atoms={(0, )}, epsilon=0.1, rmin_half=3)
        assert smirks.smirks == smirks_pattern
    else:
        with pytest.raises(error):
            _ = AtomSmirks(smirks=smirks_pattern, atoms={(0,)}, epsilon=0.1, rmin_half=3)


@pytest.mark.parametrize("smirks_data", [
    pytest.param(("[#1:1]-[#6:2]", None), id="Valid bond smirks"),
    pytest.param(("[#1]", SMIRKSParsingError), id="Missing all tags"),
    pytest.param(("[#1:1]-[#6:2]=[#6:3]", SMIRKSTypeError), id="Three tags error"),
])
def test_bond_smirks(smirks_data):
    ...
    smirks_pattern, error = smirks_data
    if error is None:
        smirks = BondSmirks(smirks=smirks_pattern, atoms={(0, 1)}, k=500, length=2)
        assert smirks.smirks == smirks_pattern
    else:
        with pytest.raises(error):
            _ = BondSmirks(smirks=smirks_pattern, atoms={(0, 1)}, k=500, length=2)
...
def test_bond_smirks(smirks_data):
    ...
    smirks_pattern, error = smirks_data
    if error is None:
        smirks = AngleSmirks(smirks=smirks_pattern, atoms={(0, 1, 2)}, k=500, angle=120)
        assert smirks.smirks == smirks_pattern
    else:
        with pytest.raises(error):
            _ = AngleSmirks(smirks=smirks_pattern, atoms={(0, 1, 2)}, k=500, angle=120)
...
def test_bond_smirks(smirks_data):
    ...
    smirks_pattern, error = smirks_data
    if error is None:
        smirks = TorsionSmirks(smirks=smirks_pattern, atoms={(0, 1, 2, 3)})
        assert smirks.smirks == smirks_pattern
    else:
        with pytest.raises(error):
            _ = TorsionSmirks(smirks=smirks_pattern, atoms={(0, 1, 2, 3)})
...
def test_atom_units():
    ...
    smirk = AtomSmirks(smirks="[#1:1]", atoms={(0, )}, epsilon=0.1, rmin_half=2)
...
def test_bond_units():
    ...
    """
    smirk = BondSmirks(smirks="[#1:1]-[#6:2]", atoms={(0, 1)}, k=500, length=2)
...
def test_angle_units():
    ...
    """
    smirk = AngleSmirks(smirks="[#1:1]-[#6:2]-[#6:3]", atoms={(0, 1, 2)}, k=100, angle=120)
...
def test_torsion_units():
    ...
    """
    smirk = TorsionTerm(periodicity=1, phase=27, k=10)
...
def test_torsion_parameterize():
    ...
    """
    smirk = TorsionSmirks(smirks="[#1:1]-[#6:2]-[#6:3]-[#1:4]", atoms={(0, 1, 2, 3)})
...
def test_torsion_term():
    ...
    """
    term = TorsionTerm(periodicity=1, phase=0)
...
def test_adding_torsion_term():
    ...
    torsion = TorsionSmirks(smirks="[#1:1]-[#6:2]-[#6:3]-[#1:4]", atoms={(0, 1, 2, 3)})
...
@pytest.mark.parametrize("smirks_data", [
    pytest.param((AtomSmirks(smirks="[#1:1]", atoms={(0, )}, epsilon=0.1, rmin_half=2), vdWHandler.vdWType), id="Atom smirks"),
    pytest.param((BondSmirks(smirks="[#1:1]-[#6:2]", atoms={(0, 1)}, k=500, length=2), BondHandler.BondType), id="Bond smirks"),
    pytest.param((AngleSmirks(smirks="[#1:1]-[#6:2]-[#6:3]", atoms={(0, 1, 2)}, k=100, angle=120), AngleHandler.AngleType), id="Angle smirks"),
    pytest.param((TorsionSmirks(smirks="[#1:1]-[#6:2]-[#6:3]-[#1:4]", atoms={(0, 1, 2, 3)}, terms={"1": TorsionTerm(periodicity=1)}), ProperTorsionHandler.ProperTorsionType), id="Torsion smirks")
])
...
@pytest.mark.parametrize("smirks", [
    pytest.param((AtomSmirks(smirks="[#1:1]", atoms={(0, )}, epsilon=0.1, rmin_half=2), AtomSmirks(smirks="[#1:1]", atoms={(0, )}, epsilon=0.1, rmin_half=2), True), id="Same smirks"),
    pytest.param((AtomSmirks(smirks="[#1:1]", atoms={(0, )}, epsilon=0.1, rmin_half=2), BondSmirks(smirks="[#1:1]-[#6:2]", atoms={(0, 1)}, k=500, length=2), False), id="Different types")
])
...
@pytest.mark.parametrize("smirks", [
    pytest.param(("[#6X4]-[#1:1]", "[#1:1]-[#6X4]", True), id="Same atom smirks reversed"),
    ...
    pytest.param(("[#6X4:2]-[#6X4:1]", "[#6X4:1]-[#6X4:2]", True), id="Same bond reversed index"),
    pytest.param(("[#6X3:1]-[#6X4:2]", "[#6X4:2]-[#6X3:1]", True), id="Same bond reversed terms"),
    pytest.param(("[#6X4:3]-[#6X4:5]", "[#6X4:1]-[#6X4:2]", False), id="Same bond wrong index"),
    pytest.param(("[#6X3:1](~[#8X1])~[#8X1:2]", "[#6X3:2](~[#8X1])~[#8X1:1]", True), id="Same bond extras"),
    ...
    pytest.param(("[#16X2,#16X1-1,#16X3+1:1]-[#6X3:2]-[#6X4:3]-[#7X3$(*-[#6X3,#6X2]):4]", "[*:1]-[#6X4:2]-[#6X4:3]-[*:4]", False), id="Different torsion"),
    pytest.param(("[#6X4]-[#1:1]", "[#6X4:1]-[#1:2]", False), id="Different no of index"),
])
