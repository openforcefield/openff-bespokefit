...
@pytest.mark.parametrize("smirks", [
    pytest.param((AtomSmirks(smirks="[#5:1]", parameterize={"rmin_half"}, atoms={(0, )}, epsilon=0.01, rmin_half=3), "n"), id="Atom smirks"),
    pytest.param((BondSmirks(smirks="[#5:1]-[#5:2]", parameterize={"k"}, atoms={(0, 1)}, k=100, length=2), "b"), id="Bond smirks"),
    pytest.param((AngleSmirks(smirks="[#5:1]-[#5:2]-[#1:3]", parameterize={"angle"}, atoms={(0, 1, 2)}, k=100, angle=120), "a"), id="Angle smirks"),
    pytest.param(
        (TorsionSmirks(smirks="[#1:1]-[#5:2]-[#5:3]-[#1:4]", parameterize={"k1"}, atoms={(0, 1, 2, 3)}, terms={"1": TorsionTerm(periodicity=1)}), "t"),
        id="Torsion smirks"),
])
...
@pytest.mark.parametrize("smirks_data", [
    ...
    pytest.param(
        ...
        id="Angle smirks"),
    pytest.param(
        (TorsionSmirks(smirks="[*:1]-[#6X4:2]-[#6X4:3]-[*:4]", atoms={(0, 1, 2, 3)},
                       terms={"3": TorsionTerm(periodicity=3)}), {"k": "0.2042684902198 * mole**-1 * kilocalorie", "phase" : "0.0 * degree"}),
        id="Torsion smirks"),
])
